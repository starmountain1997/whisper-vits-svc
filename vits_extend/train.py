...
def train(rank, device, device_num, name, chkpt_path, hp, hp_str):
    ...
    if device_num > 1:
        init_process_group(
            ...
            world_size=hp.dist_config.world_size * device_num,
            rank=rank,
        )
    ...
    trainloader = create_dataloader_train(hp, device_num, rank)
    ...
    for epoch in range(init_epoch, hp.train.epochs):
        ...
        for ppg, ppg_l, vec, pit, spk, spec, spec_l, audio, audio_l in loader:
            ...
            spk_loss = spkc_criterion(
                spk, spk_preds, torch.Tensor(spk_preds.size(0)).to(device).fill_(1.0)
            )
            # Mel Loss
            ...
            # Kl Loss
            loss_kl_f = (
                kl_loss(z_f, logs_q, m_p, logs_p, logdet_f, z_mask) * hp.train.c_kl
            )
            loss_kl_r = (
                kl_loss(z_r, logs_p, m_q, logs_q, logdet_r, z_mask) * hp.train.c_kl
            )
        ...
        if rank == 0 and epoch % hp.log.save_interval == 0:
            save_path = os.path.join(pth_dir, "%s_%04d.pt" % (name, epoch))
            torch.save(
                {
                    "model_g": (
                        model_g.module if device_num > 1 else model_g
                    ).state_dict(),
                    "model_d": (
                        model_d.module if device_num > 1 else model_d
                    ).state_dict(),
        ...
        if rank == 0:
            ...
            def clean_checkpoints(
                ...
                name_key = lambda _f: int(
                    re.compile(f"{name}_(\d+)\.pt").match(_f).group(1)
                )
                ...
                if n_ckpts_to_keep == 0:
                    ...
                    to_del = [
                        ...
                        for fn in x_sorted(f"{name}")[:-n_ckpts_to_keep]
                    ]
                del_info = lambda fn: logger.info(
